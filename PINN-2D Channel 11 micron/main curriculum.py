...
def prediction(epochs=50,maxT=3,Pe=1,initial_weights = None,train=True,plot_cv=True,saving_directory="./Data",alpha=1.0,a=0.1,b=0.1,num_train_samples = int(1e6)):
    """
    ...
    if train:
        ...
        pinn.fit(x=x_train,y=y_train,epochs=epochs,batch_size =2500,verbose=2,callbacks=[lr_scheduler_callback],shuffle=False)
        ...
        try:
            ...
            if initial_weights:
                pinn.load_weights(initial_weights)
            pinn.fit(x=x_train,y=y_train,epochs=epochs,batch_size =2500,verbose=2,callbacks=[lr_scheduler_callback])
            pinn.save_weights(f'./weights/weights {file_name}.h5')
    ...
    for index,time_sect in enumerate(time_sects):
        ...
        fig.savefig(f'{saving_directory}/Pe={Pe:.2f} epochs ={epochs} a={before_electrode:.2E} b = {after_electrode:.2E} n = {num_train_samples:.2E}.png')
    ...
    num_train_s.append(num_train_samples)
    I_PINNs.append(I_PINN*1e6)
...
if __name__ =='__main__':
    ...
    saving_directory = '11 micron data'

    

    prediction(epochs=800,maxT=maxT,Pe=startPe*0.1,train=False,initial_weights = None,saving_directory=saving_directory,a=0.3,b=0.3) # good

    prediction(epochs=800,maxT=maxT,Pe=startPe*0.2,train=False,initial_weights = None,saving_directory=saving_directory,a=0.3,b=0.3) # good  

    prediction(epochs=800,maxT=maxT,Pe=startPe*0.4,train=False,initial_weights = None,saving_directory=saving_directory,a=0.2,b=0.2)#good

    prediction(epochs=800,maxT=maxT,Pe=startPe*0.6,train=False,initial_weights = None,saving_directory=saving_directory,a=0.1,b=0.1) #good


    weights = prediction(epochs=800,maxT=maxT,Pe=startPe*0.6,train=False,initial_weights = None,saving_directory=saving_directory,a=0.1,b=0.1)

    prediction(epochs=800,maxT=maxT,Pe=startPe*0.8,train=False,initial_weights = weights,saving_directory=saving_directory,a=0.1,b=0.1)


    weights = prediction(epochs=750,maxT=maxT,Pe=startPe*0.8,train=False,initial_weights = None,saving_directory=saving_directory,a=0.1,b=0.1)
    prediction(epochs=750,maxT=maxT,Pe=startPe*1.0,train=False,initial_weights = weights,saving_directory=saving_directory,a=0.1,b=0.1)    
    ...
    df = pd.DataFrame({'Pe':Pes,'Vf':Vfs,'a':a_s,'b':b_s,'n_train_samples':num_train_s,'Flux':Fluxes,'I':I_PINNs})
